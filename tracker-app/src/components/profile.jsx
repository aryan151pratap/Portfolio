...
function Profile({ color, data }) {
	...
	useEffect(() => {
		setForm({ bio: data.bio || '', details: data.details || [] });
		setTempBio(data.bio || '');
		setTempDetails(data.details || []);
	}, [data]);
	...
	const addNewDetail = () => {
		if (newDetailText.trim() !== "") {
			const updatedDetails = [...form.details, newDetailText.trim()];
			const updatedForm = { ...form, details: updatedDetails };
			setForm(updatedForm);
